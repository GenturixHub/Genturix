      ...
      {billingOverview && billingOverview.condominiums?.length > 0 && (
        <Card className="bg-[#0F111A] border-[#1E293B]">
          <CardHeader className="pb-3">
            ...
            <CardDescription>
              Estado de asientos y suscripciones por condominio
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-3">
              {billingOverview.condominiums.map((condo) => (
                <div 
                  key={condo.condominium_id}
                  className="p-3 rounded-lg bg-[#1E293B]/30 border border-[#1E293B]"
                >
                  <div className="flex justify-between items-start mb-2">
                    <div>
                      <p className="font-medium">{condo.condominium_name}</p>
                    ...
                    <Badge 
                      ...
                      className={
                        ...
                        condo.billing_status === 'trialing' ? 'text-blue-400 border-blue-400/30' :
                        condo.billing_status === 'past_due' ? 'text-yellow-400 border-yellow-400/30' :
                        'text-red-400 border-red-400/30'
                      }
                    >
                      {condo.billing_status === 'active' ? 'Activo' :
                       condo.billing_status === 'trialing' ? 'Prueba' :
                       condo.billing_status === 'past_due' ? 'Pago Pendiente' :
                       'Cancelado'}
                    </Badge>
                  </div>
                  ...
                  <div className="flex items-center gap-4 text-sm">
                    <div className="flex items-center gap-1">
                      ...
                      <span>{condo.active_users}/{condo.paid_seats} usuarios</span>
                    </div>
                    ...
                    <span className={`text-xs ${condo.remaining_seats <= 0 ? 'text-red-400' : 'text-muted-foreground'}`}>
                      {condo.remaining_seats} disponibles
                    </span>
                  </div>
                </div>
              ))}
            ...
            {/* Totals Summary */}
            <div className="mt-4 pt-4 border-t border-[#1E293B] grid grid-cols-3 gap-4 text-center">
              <div>
                <p className="text-2xl font-bold text-emerald-400">{billingOverview.totals?.total_condominiums}</p>
                <p className="text-xs text-muted-foreground">Condominios</p>
              </div>
              <div>
                <p className="text-2xl font-bold text-purple-400">{billingOverview.totals?.total_active_users}/{billingOverview.totals?.total_paid_seats}</p>
                <p className="text-xs text-muted-foreground">Usuarios/Asientos</p>
              </div>
              <div>
                <p className="text-2xl font-bold text-green-400">${billingOverview.totals?.total_monthly_revenue?.toFixed(2)}</p>
